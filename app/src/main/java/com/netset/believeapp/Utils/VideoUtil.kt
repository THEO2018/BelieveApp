...
object VideoUtil {
    var DIR_NAME = "BelieveAppPrivate"

    fun getVideoStoragePath(activity: BaseActivity): File {
        val root = File(
...
//        val sub = File(activity.filesDir, DIR_NAME)
        if (!root.exists()) {
            root.mkdirs()
        } else {
        }
    ...
    fun compressor(inputVideo: String, outputVideo: String): Array<String> {
        getFrameRate(inputVideo)
    ...
    fun getFilePath(context: Context, fileExtension: String) : String {
        val dir = File(context.getExternalFilesDir(OUT_PUT_DIR).toString())
        if (!dir.exists()) {
            dir.mkdirs()
        }
        ...
        val dest = File(dir.path + File.separator + OUT_PUT_DIR + System.currentTimeMillis().div(1000L) + extension)
